...
ret = subprocess.call(
    ["ping", "-w", "100", "google.com"], stdout=devnull, stderr=devnull
)
...
def touch_env():
    ...
    if not os.path.exists(os.path.join(env_dir, "tools", "scripts")):
        try:
            ret = os.system(
                "git clone %s %s" % (env_url, os.path.join(env_dir, "tools", "scripts"))
            )
            if ret != 0:
    ...
    if sys.platform != "win32":
        ...
        if os.path.exists(os.path.join(env_dir, "tools", "scripts")):
            os.environ["PATH"] = (
                os.path.join(env_dir, "tools", "scripts") + ";" + os.environ["PATH"]
            )
